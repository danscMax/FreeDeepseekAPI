...
// Only accept a wasmUrl that is HTTPS, on a deepseek.com host, and a .wasm path.
// The JSON-import branch passes wasmUrl through verbatim, and the server later
// fetch()es + executes it (PoW solver) — an unrestricted value is an SSRF +
// arbitrary-WASM vector (e.g. pointing at cloud metadata). Reject anything else
// and fall back to a trusted default.
function isAllowedWasmUrl(u) {
    if (!u) return false;
    try {
...
function finalizeAuth(parsed, prevWasmUrl) {
    ...
    // Use the supplied wasmUrl only if it passes the allowlist; otherwise the
    // previous account's URL, else the trusted default.
    const wasmUrl = isAllowedWasmUrl(parsed.wasmUrl)
        ? parsed.wasmUrl
        : (isAllowedWasmUrl(prevWasmUrl) ? prevWasmUrl : WASM_DEFAULT);
    return {
        token: parsed.token,
...
module.exports = { parseCurl, parseHar, parseAuthInput, finalizeAuth, isAllowedWasmUrl, WASM_DEFAULT };
